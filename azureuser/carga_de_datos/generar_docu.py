import requests
from ejecuciones_iniciales.config import nombres  # Importar la lista nombres desde config.py
...
def consulta_api(anno, tipo_demanda):
    try:
        api_url = f"https://apidatos.ree.es/es/datos/{tipo_demanda[0]}/{tipo_demanda[1]}?end_date={anno}-12-15T23:59&time_trunc={tipo_demanda[2]}&start_date={anno}-11-20T04:00"
...
for elemento in nombres:
    data_type, description = consulta_api(2020, elemento)
    # if data_type is not None and description is not None:
    resultados.append(f"Categoria: {elemento[0]}\nElemento: {elemento[1]}\nDescripcion: {data_type}\n\n")
