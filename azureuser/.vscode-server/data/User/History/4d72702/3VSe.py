...
def demanda(anno, tipo_demanda):
    api_url = f"https://apidatos.ree.es/es/datos/demanda/{tipo_demanda}?end_date={anno}-12-31T23:59&time_trunc=day&start_date={anno}-01-01T04:00"
...
def demanda_ccaa(anno, geo_name, geo_id, tipo_demanda):
    # Parámetros de la API
        api_url = f"https://apidatos.ree.es/es/datos/demanda/{tipo_demanda}?end_date={anno}-12-31T23:59&time_trunc=month&start_date={anno}-01-01T04:00&geo_trunc=electric_system&geo_limit=ccaa&geo_ids={geo_id}"
...
for anno in range(2022, 2023):
    ...
    tipo_demanda = {"demanda":"evolucion", "demanda":"perdidas-transporte", "demanda":"demanda-maxima-diaria"}
    for tipo in tipo_demanda:
        print(tipo)
    #     demanda(anno, tipo)
    # comunidades(anno, "evolucion")
