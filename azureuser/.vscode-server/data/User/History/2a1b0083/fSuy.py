        ...
        if year != "COMUNIDAD AUTONOMA":
            ...
            for num_sensor in range(1, 6):
                ...
                for dia in pd.date_range(start=f"{year}-01-01", end=f"{year}-12-31"):
                    ...
                    registros.append({"Fecha": dia, "Comunidad Autónoma": comunidad, "Sensor": f"Sensor {num_sensor}", "Valor": valor_sensor})
        ...
        if year != "COMUNIDAD AUTONOMA":
            for num_sensor in range(1, 6):
                suma_sensores = df_final[(df_final["Fecha"].dt.year == int(year)) & 
                                         (df_final["Comunidad Autónoma"] == comunidad) & 
                                         (df_final["Sensor"] == f"Sensor {num_sensor}")]["Valor"].sum()
                print(f"Año: {year}, Comunidad Autónoma: {comunidad}, Sensor: {num_sensor}, Suma de sensores: {suma_sensores}, Valor inicial: {value}")
# Mostrar el DataFrame resultante
print(df_final)
