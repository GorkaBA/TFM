...
nombres = [
    ...
    ("demanda", "perdidas-transporte", "month", True),
    ...
    ("generacion", "no-renovables-detalle-emisiones-CO2", "month", True),
    ("generacion", "maxima-renovable", "month", True),
    ("generacion", "potencia-instalada", "month", True),
    ("generacion", "maxima-renovable-historico", "month", True),
    ("generacion", "maxima-sin-emisiones-historico", "month", True)
]
...
def demanda(anno, tipo_demanda):
    ...
    try:
        ...
        print(f"Error: La respuesta de la API no es un JSON válido para el año {anno}.")
        return

    # Procesar y escribir datos en InfluxDB
    for item in data.get('included', []):
        for value in item.get('attributes', {}).get('values', []):
...
def demanda_ccaa(anno, geo_name, geo_id, tipo_demanda):
    ...
    api_url = f"https://apidatos.ree.es/es/datos/{tipo_demanda[0]}/{tipo_demanda[1]}?end_date={anno}-12-31T23:59&time_trunc={tipo_demanda[2]}&start_date={anno}-01-01T04:00&geo_trunc=electric_system&geo_limit=ccaa&geo_ids={geo_id}"
    ...
    try:
        ...
        print(f"Error: La respuesta de la API no es un JSON válido para el año {anno}, geo_id {geo_id}.")
        return
