...
def demanda_hour(anno, tipo_demanda):
    for mes in range(1, 13):
        ...
        if mes == 12:
            end_date = datetime(anno + 1, 1, 1) - timedelta(seconds=1)
        else:
            end_date = datetime(anno, mes + 1, 1) - timedelta(seconds=1)
        ...
        if response.status_code != 200:
            print(f"Error al obtener datos de la API para el mes {mes} del año {anno}:", response.status_code)
            ...
            print(f"Datos para el mes {mes} del año {anno} {tipo_demanda[0]}/{tipo_demanda[1]} escritos en InfluxDB con éxito.")
