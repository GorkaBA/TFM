...
def descargar_datos_historicos(tipo_demanda):
    ...
    print("Verificando formato de fechas antes de la conversión:", df['ds'].head())
    df['ds'] = pd.to_datetime(df['ds'], errors='coerce')  # Convertir a datetime
    print("Verificando formato de fechas después de la conversión:", df['ds'].head())
    df['ds'] = df['ds'].dt.tz_convert('UTC').dt.tz_localize(None)  # Convertir a UTC y remover la info de zona horaria
    return df
