...
import numpy as np
from config import influxdb_token, influxdb_org, influxdb_url, comunidades
...
# Calcular fecha de inicio (hoy - 6 años)
end_date = datetime.utcnow()
start_date = end_date - timedelta(days=365 * 6)  # 6 años atrás
...
for comunidad in comunidades:
    ...
    from(bucket:"{influxdb_bucket}") 
      ...
      |> filter(fn: (r) => r["_measurement"] == "mensual" and r["_field"] == "CO2_ajustado_solo_generacion" and r["Comunidad"] == "{comunidad}")
    '''
    ...
    print(f"Datos descargados para {comunidad}:")
    ...
    # Si no hay datos, continuar con la siguiente comunidad
    if not timestamps:
        print(f"No hay datos para {comunidad}.")
        continue

    # Obtener el último timestamp y calcular los dos siguientes meses
    last_timestamp = max(timestamps)
    next_months = [
        last_timestamp + relativedelta(months=2, day=1, hour=1, minute=0, second=0, microsecond=0),
        last_timestamp + relativedelta(months=3, day=1, hour=1, minute=0, second=0, microsecond=0),
        last_timestamp + relativedelta(months=4, day=1, hour=1, minute=0, second=0, microsecond=0)
    ]
    ...
    # Transformar características a polinómicas
    poly = PolynomialFeatures(degree=3)  # Ajusta el grado del polinomio según sea necesario
    timestamps_poly = poly.fit_transform(timestamps_combined)

    # Construcción del modelo de predicción con términos polinómicos
    ...
    for next_month in next_months:
        ...
        next_month_poly = poly.transform(next_month_combined)
        predicted_value = model.predict(next_month_poly)
    ...
    for i, timestamp in enumerate(next_months):
        point = Point("mensual") \
            ...
            .field("CO2_Prediccion2", predicted_values[i][0])
