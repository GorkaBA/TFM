...
# Obtener el último timestamp y calcular el siguiente mes
last_timestamp = max(timestamps)
next_month = last_timestamp + relativedelta(months=1)
next_month_timestamps = [next_month.replace(day=1, hour=1, minute=0, second=0, microsecond=0)]  # Primero del siguiente mes a las 01:00:00
...
encoder = OneHotEncoder(categories=[np.arange(1, 13)], sparse=False)
...
for i, timestamp in enumerate(next_month_timestamps):
    ...
    print("Fecha")
    print(int(timestamp.timestamp() * 1e9))
